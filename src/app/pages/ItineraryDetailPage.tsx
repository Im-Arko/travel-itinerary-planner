...
export function ItineraryDetailPage() {
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">{error || 'Itinerary not found.'}</h2>
          <Link to="/itineraries" className="text-blue-600 hover:text-blue-700">Back to Saved Itineraries</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)]">
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-700 to-purple-700 px-4 py-16 text-white">
        <Link to="/itineraries" className="absolute top-6 left-6 flex items-center gap-2 px-4 py-2 bg-white/90 text-gray-800 backdrop-blur-sm rounded-lg hover:bg-white transition-colors">
          <ArrowLeft className="w-5 h-5" />
          <span className="font-medium">Back</span>
        </Link>

        <button onClick={toggleFavorite} className="absolute top-6 right-6 p-3 bg-white/90 backdrop-blur-sm rounded-full hover:bg-white transition-colors">
          <Heart className={`w-6 h-6 ${itinerary.is_favorite ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} />
        </button>

        <div className="max-w-7xl mx-auto pt-10">
          <h1 className="text-5xl font-bold mb-3">{itinerary.title}</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl p-8 mb-8">
          <div className="grid md:grid-cols-3 gap-6">
            <div><p className="text-blue-100 mb-1">Duration</p><p className="text-3xl font-bold">{itinerary.duration_days} Days</p></div>
            <div><p className="text-blue-100 mb-1">Budget</p><p className="text-3xl font-bold capitalize">{itinerary.budget}</p></div>
            <div><p className="text-blue-100 mb-1">Created On</p><p className="text-3xl font-bold">{new Date(itinerary.generated_at).toLocaleDateString()}</p></div>
          </div>
          {itinerary.summary && <p className="mt-6 text-blue-50">{itinerary.summary}</p>}
        </div>

        <div className="space-y-6">
          {itinerary.days.map((day) => (
            <div key={day.day_number} className="bg-white rounded-2xl shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-4">
                <h2 className="text-2xl font-bold">Day {day.day_number}</h2>
              ...
              <div className="p-6 space-y-6">
                ...
                <ActivityBlock icon={<Moon className="w-5 h-5 text-purple-500" />} title="Evening" value={day.evening} accent="border-purple-400" />
